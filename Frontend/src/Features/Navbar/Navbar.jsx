    ...
    <div className="navbar-wrapper flex flex-row items-center justify-center">
      <div className="navbar bg-[#3b3b3b] w-[calc(100%-20px)] mt-[10px] rounded-lg h-[70px] box-border px-5 text-white flex items-center justify-between gap-4 relative z-50">
        ...
        <div className="flex items-center gap-3">
          ...
          <input
            type="text"
            className={`h-9 w-[165px] sm:w-[300px] rounded-md bg-[#F3F4F6] outline-none text-black px-2 transition-transform duration-300 ease-out ${
              showSearchBar
            ...
            >
              {profileOptions.map((option) => (
                <Link key={uuid()} to={option.link}>
                  <div className=" flex h-[40px] items-center cursor-pointer text-slate-700 pl-4 text-md font-normal hover:bg-gray-200">
                    {option.name}
                  </div>
                </Link>
              ))}
          ...
          <ul
            ...
            } burger-menu md:hidden w-full sm:w-[40%] bg-[#3b3b3b] rounded-md absolute top-[100%] right-0 z-100 `}
          >
            {mobileViewMenuOption.map((link) => (
              <Link key={uuid()} to={link.link}>
                <div className="h-7 flex items-center p-4 py-6 font-semibold">
                  {link.name}
                </div>
              </Link>
            ))}
